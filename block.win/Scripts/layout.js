...
if (TradePasswordIsSet ===true) {
    $('.traPasswodIsSet').text('原交易密码');
    $('.traPasswodPlachole').attr('placeholder', '原交易密码');
} else {
    $('.traPasswodIsSet').text('登录密码');
    $('.traPasswodPlachole').attr('placeholder', '登录密码');
}
...
//$('.modal').on('show.bs.modal', function () { $('body').css('margin-right', '+=17'); $('#navvv').css('right', '+=17') }).on('hidden.bs.modal', function () { $('body').css('margin-right', '-=17'); $('#navvv').css('right', '-=17') })
...
function SendForgotTradePasswordEmail() {
    server.SendForgotTradePasswordEmail({
        ...
        if (rel.msg === 'StatusOk') {
            ...
            showAlertBox('ForgotTradePasswordAlertBox', '找回交易密码邮件已发送，请注意查收');
            var cd = setInterval(function () {
                $('#SendForgotTradePasswordBtn').text(countdown + 'S后重新发送').removeClass('bg-10ADE4').addClass('bg-ccc');
                if (countdown == 0) {
                    clearInterval(cd);
                    $('#SendForgotTradePasswordBtn').text('发送验证码').removeClass('bg-ccc').addClass('bg-10ADE4').removeAttr('disabled');
                }
                countdown--;
            }, 1000);
        } else {
...
function ForgotTradePassword() {
    ...
    if (!Code) {
        showAlertBox('ForgotTradePasswordAlertBox','请输入邮箱验证码');
        return;
    }
    if (!newTradePassword) {
        showAlertBox('ForgotTradePasswordAlertBox','请输入新交易密码');
        return;
    }
    if (!ConfirmNewTradePassword) {
        showAlertBox('ForgotTradePasswordAlertBox','请确认新交易密码');
        return;
    }
    server.ForgotTradePassword({
        ...
        if (rel.msg === 'StatusOk') {
            showAlertBox('ForgotTradePasswordAlertBox','修改成功');
            setTimeout(function () {
...
function ChangeLoginPassword() {
    ...
    server.ChangeLoginPassword({
        ...
        switch (rel.msg) {
            case 'StatusOk': {
                showAlertBox('ChangeLoginPassword', '修改成功');
                $('#OldePassword').val('');
                $('#newPassword').val('');
            ...
            default: {
                showAlertBox('ChangeLoginPassword', '修改失败，错误码：'+rel.msg);
            }
        }
    });
}
...
function InitTradePassword() {
    ...
    if (TradePasswordIsSet === true) {
        server.ChangeTradePassword({
            ...
            switch (rel.msg) {
                case 'StatusOk': {
                    showAlertBox('InitTradePasswordAlertBox', '修改成功');
                    $('#OldPassword').val('');
                    $('#Password').val('');
                    $('#ConfirmPassword').val('');
                    setTimeout(function () {
                ...
                default: {
                    showAlertBox('InitTradePasswordAlertBox', '修改失败，错误码：' + rel.msg);
                }
            }
        });
    } else {
        server.InitTradePassword({
            ...
            switch (rel.msg) {
                case 'StatusOk': {
                    showAlertBox('InitTradePasswordAlertBox', '设置成功');
                    $('#OldPassword').val('');
                    $('#Password').val('');
                    $('#ConfirmPassword').val('');
                    setTimeout(function () {
                ...
                default: {
                    showAlertBox('InitTradePasswordAlertBox', '设置失败，错误码：' + rel.msg);
                }
            }
        });
    }
...
function Logout() {
    server.Logout({}, function () {
        window.location.href = host + '/zh-CN';
    })
}
...
function UpdateUserInfo() {
    ...
    server.UpdateUserInfo({
        ...
        switch (rel.msg) {
            case 'StatusOk': {
                showAlertBox('UpdateUserInfoAlertBox', '修改成功');
                setTimeout(function () {
            ...
            default: {
                showAlertBox('UpdateUserInfoAlertBox', '修改失败，错误码：' + rel.msg);
            }
        }
    })
}
